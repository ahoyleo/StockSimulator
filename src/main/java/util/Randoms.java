...
public class Randoms extends java.util.Random
{
  ...
  /**
   * Return random integer from Poisson with parameter lambda. The mean of this
   * distribution is lambda. The variance is lambda.
   */
  ...
  /**
   * Return a random double in the range 0 to 1, inclusive, uniformly sampled from that
   * range. The mean of this distribution is 0.5. The variance is 1/12.
   */
  ...
  /**
   * Return a random double in the range a to b, inclusive, uniformly sampled from that
   * range. The mean of this distribution is (b-a)/2. The variance is (b-a)^2/12
   */
  ...
  /**
   * draw a single sample from (unnormalized) multinomial "a", with normalizing factor
   * "sum".
   */
  ...
  /**
   * Return a random double drawn from a Gaussian distribution with mean m and variance
   * s2.
   */
  ...
  // E(X)=1 ; Var(X)=1
  /**
   * Return a random double drawn from a Gamma distribution with mean 1.0 and variance
   * 1.0.
   */
  ...
  /**
   * Return a random double drawn from a Gamma distribution with mean alpha and variance
   * 1.0.
   */
  ...
  /**
   * Return a random double drawn from a Gamma distribution with mean alpha*beta and
  ...
  /**
   * Return a random double drawn from a Gamma distribution with mean alpha*beta+lamba and
   * variance alpha*beta^2.
   */
  ...
  /**
   * Return a random double drawn from an Exponential distribution with mean 1 and
   * variance 1.
   */
  ...
  /**
   * Return a random double drawn from an Exponential distribution with mean beta and
   * variance beta^2.
   */
  ...
  /**
   * Return a random double drawn from an Exponential distribution with mean beta+lambda
   * and variance beta^2.
   */
  ...
  /**
   * Return a random double drawn from an Chi-squarted distribution with mean 1 and
   * variance 2. Equivalent to nextChiSq(1)
   */
  ...
  /**
   * Return a random double drawn from an Chi-squared distribution with mean df and
   * variance 2*df.
   */
  ...
  /**
   * Return a random double drawn from an Chi-squared distribution with mean df+lambda and
   * variance 2*df.
   */
  ...
  /**
   * Return a random double drawn from a Beta distribution with mean a/(a+b) and variance
   * ab/((a+b+1)(a+b)^2).
   */
  public synchronized double nextBeta(double alpha, double beta)
  {
    ...
    {
      double A = alpha - 1, B = beta - 1, C = A + B, L = C * Math.log(C), mu = A / C, sigma =
          0.5 / Math.sqrt(C);
      ...
      double u = nextUniform();
      while (Math.log(u) >= A * Math.log(x / A) + B * Math.log((1 - x) / B) + L + 0.5 * y
          * y)
      {
        y = nextGaussian();
        x = sigma * y + mu;
    ...
    {
      double v1 = Math.pow(nextUniform(), 1 / alpha), v2 =
          Math.pow(nextUniform(), 1 / beta);
      while (v1 + v2 > 1)
      {
  ...
  public static void main(String[] args)
  {
    ...
    System.out.println(r.nextPoisson());
